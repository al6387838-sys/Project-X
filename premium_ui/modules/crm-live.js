...
(() => {
  ...
  function icons() {
    try { window.refreshIcons?.(); } catch (_) { /* ícones não impedem interação */ }
  }
  ...
  function scopeBar() {
    ...
    return `<div class="card" style="padding:12px 14px;margin-bottom:16px;display:flex;flex-wrap:wrap;align-items:center;gap:10px">
      ...
      ${capabilities().workspace ? '<button id="crm-new-workspace" class="btn btn-ghost" style="margin-left:auto"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Workspace</button>' : ''}
    </div>`;
  }
  ...
  function emptyState(title, copy, actionLabel, action) {
    return `<div style="padding:34px 18px;text-align:center;color:var(--text-muted)">
      ...
      ${actionLabel && capabilities().write ? `<button class="btn btn-primary" data-action="${escapeAttr(action)}"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> ${escapeHtml(actionLabel)}</button>` : ''}
    </div>`;
  }
  ...
  function onboardingView() {
    return `<div class="page-header"><div><div class="page-title"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/></svg> CRM Enterprise</div><div class="page-subtitle">Crie uma organização para iniciar o espaço comercial colaborativo.</div></div></div>
      <div class="card" style="max-width:700px;padding:28px"><div style="font-size:18px;font-weight:800;margin-bottom:8px">Seu CRM está pronto para uma organização real</div><p style="margin:0 0 18px;color:var(--text-secondary);font-size:13px;line-height:1.6">Clientes, oportunidades, agenda e histórico serão armazenados somente após a criação de uma organização e de seu workspace inicial.</p><button class="btn btn-primary" data-action="create-organization"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Criar organização</button></div>`;
  }
  ...
  function renderClients() {
    ...
    root.innerHTML = `${scopeBar()}
      <div class="page-header"><div><div class="page-title"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 21a8 8 0 0 0-16 0"/><circle cx="10" cy="8" r="5"/><path d="M22 20c0-3.37-2-6.5-4-8a5 5 0 0 0-.45-8.3"/></svg> CRM</div><div class="page-subtitle">Clientes, histórico e relacionamento comercial de ${escapeHtml(state.data.organization.name)}</div></div><div style="display:flex;gap:8px"><button class="btn btn-ghost" data-action="show-agenda"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/><path d="M16 18h.01"/></svg> Agenda</button><button class="btn btn-ghost" data-action="show-pipeline">Pipeline</button>${capabilities().write ? '<button class="btn btn-primary" data-action="new-contact"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Cliente</button>' : ''}</div></div>
      <div class="grid-4" style="margin-bottom:20px">
        <div class="card kpi-card"><div class="kpi-icon" style="background:rgba(99,102,241,.15);color:var(--accent)"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 21a8 8 0 0 0-16 0"/><circle cx="10" cy="8" r="5"/><path d="M22 20c0-3.37-2-6.5-4-8a5 5 0 0 0-.45-8.3"/></svg></div><div class="kpi-value">${metrics.contactCount || 0}</div><div class="kpi-label">Clientes</div></div>
        <div class="card kpi-card"><div class="kpi-icon" style="background:rgba(16,185,129,.15);color:var(--green)"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12h.01"/><path d="M16 6V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><path d="M22 13a18.15 18.15 0 0 1-20 0"/><rect x="2" y="6" width="20" height="14" rx="2"/></svg></div><div class="kpi-value">${metrics.opportunityCount || 0}</div><div class="kpi-label">Oportunidades</div></div>
        <div class="card kpi-card"><div class="kpi-icon" style="background:rgba(245,158,11,.15);color:var(--amber)"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2"/><path d="M3 11h3c.8 0 1.6.3 2.1.9l1.1.9c1.6 1.6 4.1 1.6 5.7 0l1.1-.9c.5-.5 1.3-.9 2.1-.9H21"/></svg></div><div class="kpi-value" style="font-size:18px">${money(metrics.pipelineTotal)}</div><div class="kpi-label">Pipeline aberto</div></div>
        <div class="card kpi-card"><div class="kpi-icon" style="background:rgba(16,185,129,.15);color:var(--green)"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg></div><div class="kpi-value" style="font-size:18px">${money(metrics.wonTotal)}</div><div class="kpi-label">Fechado ganho</div></div>
      </div>
      <div style="display:grid;grid-template-columns:minmax(0,1fr) 330px;gap:16px;align-items:start">
        ...
        <aside class="card" style="padding:16px"><div style="font-weight:800;font-size:14px;margin-bottom:8px"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg> Histórico comercial</div>${historyRows()}</aside>
      </div>`;
    bindPageEvents();
    icons();
  }
  ...
  function renderPipeline() {
    ...
    root.innerHTML = `${scopeBar()}<div class="page-header"><div><div class="page-title"><svg  class="pg-icon" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12h.01"/><path d="M16 6V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><path d="M22 13a18.15 18.15 0 0 1-20 0"/><rect x="2" y="6" width="20" height="14" rx="2"/></svg> Pipeline de Vendas</div><div class="page-subtitle">Arraste uma oportunidade entre os estágios para atualizar o pipeline.</div></div><div style="display:flex;gap:8px"><button class="btn btn-ghost" data-action="show-crm"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg> CRM</button><button class="btn btn-ghost" data-action="show-agenda"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/><path d="M16 18h.01"/></svg> Agenda</button>${capabilities().write ? '<button class="btn btn-primary" data-action="new-deal"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Oportunidade</button>' : ''}</div></div><div style="display:grid;grid-template-columns:repeat(${Math.max(stages.length, 1)},minmax(235px,1fr));gap:10px;min-height:calc(100vh - 240px);overflow-x:auto;padding-bottom:4px">${stages.map((stage) => { const deals = (state.data.deals || []).filter((deal) => deal.stage === stage.id); return `<section class="kanban-col" data-stage-id="${escapeAttr(stage.id)}" style="min-width:235px"><div class="kanban-col-header" style="border-top-color:${escapeAttr(stage.color)}"><span>${escapeHtml(stage.name)}</span><span class="kanban-count">${deals.length}</span></div><div class="kanban-cards crm-dropzone" data-stage-id="${escapeAttr(stage.id)}" style="min-height:170px">${deals.map((deal) => dealCard(deal, stage)).join('') || `<div style="font-size:12px;color:var(--text-muted);padding:12px 2px">Sem oportunidades</div>`}</div>${capabilities().write ? `<button class="kanban-add-btn" data-action="new-deal" data-stage="${escapeAttr(stage.id)}"><svg  class="pg-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg> Adicionar</button>` : ''}</section>`; }).join('')}</div>`;
    bindPageEvents();
    bindPipelineDnd();
    icons();
  }
