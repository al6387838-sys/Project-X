...
class PriorityEngine:
    ...
    def calculate_mission_priority(self, mission: Mission) -> int:
        ...
        profile = self.personal_dna.get_profile()
        if profile and "values" in profile:
            values = profile["values"]
            objective_lower = mission.objective.lower()
            title_lower = mission.title.lower()
            if values.get("family") and ("family" in objective_lower or "família" in objective_lower or "família" in title_lower):
                base_priority += 20
            if values.get("career_growth") and ("career" in objective_lower or "carreira" in objective_lower or "trabalho" in title_lower):
                base_priority += 15
