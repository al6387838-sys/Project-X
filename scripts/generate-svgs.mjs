// Use lucide's own createElement to generate consistent SVG strings
// Then serialize to HTML string
import { createElement, icons } from 'lucide';
import { writeFileSync } from 'fs';

const toPascal = (name) => name.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('');

function iconToSvgString(iconName) {
  const pascalName = toPascal(iconName);
  const iconData = icons[pascalName];
  if (!iconData) return null;
  
  // Use lucide's createElement which returns a DOM element
  // We need to serialize it to string
  // Since we're in Node, we'll manually build the SVG string from the icon data
  ...
  const strokeLinecap = attrs.strokeLinecap || 'round';
  const strokeLinejoin = attrs.strokeLinejoin || 'round';
  
  // Recursively serialize the icon tree
  function serializeNode(node) {
    if (!node) return '';
    const [tag, nodeAttrs, nodeChildren] = node;
    if (!tag || tag === 'svg') return '';
    
    let attrStr = '';
    if (nodeAttrs.d) attrStr += ` d="${nodeAttrs.d}"`;
    if (nodeAttrs.cx !== undefined) attrStr += ` cx="${nodeAttrs.cx}"`;
    if (nodeAttrs.cy !== undefined) attrStr += ` cy="${nodeAttrs.cy}"`;
    if (nodeAttrs.r !== undefined) attrStr += ` r="${nodeAttrs.r}"`;
    if (nodeAttrs.x1 !== undefined) attrStr += ` x1="${nodeAttrs.x1}"`;
    if (nodeAttrs.y1 !== undefined) attrStr += ` y1="${nodeAttrs.y1}"`;
    if (nodeAttrs.x2 !== undefined) attrStr += ` x2="${nodeAttrs.x2}"`;
    if (nodeAttrs.y2 !== undefined) attrStr += ` y2="${nodeAttrs.y2}"`;
    if (nodeAttrs.points) attrStr += ` points="${nodeAttrs.points}"`;
    if (nodeAttrs.x !== undefined) attrStr += ` x="${nodeAttrs.x}"`;
    if (nodeAttrs.y !== undefined) attrStr += ` y="${nodeAttrs.y}"`;
    if (nodeAttrs.width !== undefined) attrStr += ` width="${nodeAttrs.width}"`;
    if (nodeAttrs.height !== undefined) attrStr += ` height="${nodeAttrs.height}"`;
    if (nodeAttrs.rx !== undefined) attrStr += ` rx="${nodeAttrs.rx}"`;
    if (nodeAttrs.fill) attrStr += ` fill="${nodeAttrs.fill}"`;
    if (nodeAttrs.transform) attrStr += ` transform="${nodeAttrs.transform}"`;
    
    // Check if it's a self-closing tag
    const selfClosing = ['path', 'circle', 'line', 'polyline', 'polygon', 'rect', 'ellipse', 'stop', 'use'];
  ...
  if (children && children.length > 0) {
    for (const child of children) {
      innerSvg += serializeNode(child);
    }
  }
  
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="${viewBox}" fill="none" stroke="${stroke}" stroke-width="${strokeWidth}" stroke-linecap="${strokeLinecap}" stroke-linejoin="${strokeLinejoin}">${innerSvg}</svg>`;
}

// All unique icon names used in the project
const iconNames = [
...
for (const name of iconNames) {
  const svg = iconToSvgString(name);
  if (svg) {
    svgMap[name] = svg;
  } else {
    missing.push(name);
